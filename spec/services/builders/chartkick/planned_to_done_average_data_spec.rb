...
describe Builders::Chartkick::PlannedToDoneAverageData do
  ...
  describe '.build_data' do
    context 'for a given set of metrics' do
      ...
      let(:average) do
        (((jira_sprint1.points_completed / jira_sprint1.points_committed.to_f).round(2) +
        (jira_sprint2.points_completed / jira_sprint2.points_committed.to_f).round(2)) * 100)
          .round / 2
      end
