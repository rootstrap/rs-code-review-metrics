RSpec.shared_examples 'user project example' do |metric_type|
  ...
  it 'returns the correct number of metrics based the amount of user in the project' do
    expect(described_class.call(project_id, metric_type).count).to eq(2)
  end

  it 'returns only data with dates between today and the past 14 days' do
    described_class.call(project_id, metric_type).each do |metric_data|
