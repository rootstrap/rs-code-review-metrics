module CodeClimate
  class RepositoriesDetailsService < BaseService
    ...
    def call
      code_climate_metrics.map do |metric|
        RepositorySummary.new(rate: metric.code_climate_rate,
    ...
    def metrics_in_time_period
      if from.positive?
        metrics_in_department.merge(Repository.with_activity_after(from.weeks.ago))
                             .group('code_climate_repository_metrics.id')
